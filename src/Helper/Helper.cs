...
public class Helper
{
    ...
    public static void TestMenu(Perceptron perceptron)
    {
        ...
        do
        {
            ...
            {
                weights[indexCurrent] = Double.Parse(weights[indexCurrent].ToString() + (commaNow ? "." : "") + (key.Key - ConsoleKey.D0), System.Globalization.NumberStyles.Float);
            ...
            {
                weights[indexCurrent] = Double.Parse(weights[indexCurrent].ToString() + (commaNow ? "." : "") + (key.Key - ConsoleKey.NumPad0), System.Globalization.NumberStyles.Float);
                menu[indexCurrent] = "w" + (indexCurrent + 1) + " = " + weights[indexCurrent];
                if (commaNow)
                {
                    commaNow = false;
            ...
            else if (numericalMode && key.Key == ConsoleKey.Backspace)
            {
                ...
                else
                {
                    try
                    {
                        weights[indexCurrent] = Double.Parse(weights[indexCurrent].ToString()[..^1], System.Globalization.NumberStyles.Float);
                    }
                    catch (Exception)
                    {
    ...
    public static void Credit()
    {
        ...
        Console.Write("Oleksandr Volkodav, s28021, for PACK, 2023/2024");
